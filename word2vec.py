...
logging.basicConfig(level=logging.DEBUG,
                ...
                filename='tmp/logs/word2vec_debug.log',
                filemode='w')

def load_word2vec():
    if config.WORD2VEC_MODEL == "google_news":
        from gensim.models.keyedvectors import KeyedVectors
        word_vectors = KeyedVectors.load_word2vec_format(config.WORD2VEC_MODEL_PATH, binary=True)
        return list(range(0, word_vectors.corpus_count)), word_vectors
    else:  # wiki
        import numpy as np
...
def get_vector_by_word(word, wordvec_index, word_vectors):
    ...
    if config.WORD2VEC_MODEL == "google_news":
        return word_vectors.wv.get_vector(word)
    else:
...
def get_vectorid_by_word(word, wordvec_index):
    if config.WORD2VEC_MODEL != "wiki":
        raise ValueError("google news vector is not supported for now")
